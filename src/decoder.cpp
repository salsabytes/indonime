#include <pybind11/pybind11.h>
#include <string>
#include <cstdint>
#include <cstring>

namespace py = pybind11;

static const uint8_t sbox[256] = {
  
};

#define F2(x) ((x << 1) ^ (((x >> 7) & 1) * 0x1b))
#define F3(x) (F2(x) ^ x)

void aes_block(uint8_t* s, const uint8_t* rk) {
  for (int r = 0; ; r++) {
    for (int i = 0; i < 16; i++) s[i] ^= rk[r * 16 + i];
    if (r == 10) break;
    for (int i = 0; i < 16; i++) s[i] = sbox[s[i]];
    uint8_t t = s[1]; s[1]=s[5]; s[5]=s[9]; s[9]=s[13]; s[13]=t;
    t=s[2]; s[2]=s[10]; s[10]=t; t=s[6]; s[6]=s[14]; s[14]=t;
    t=s[15]; s[15]=s[11]; s[11]=s[7]; s[7]=s[3]; s[3]=t;
    if (r < 9) {
      for (int i = 0; i < 16; i += 4) {
        uint8_t a=s[i], b=s[i+1], c=s[i+2], d=s[i+3];
        s[i]   = F2(a) ^ F3(b) ^ c ^ d;
        s[i+1] = a ^ F2(b) ^ F3(c) ^ d;
        s[i+2] = a ^ b ^ F2(c) ^ F3(d);
        s[i+3] = F3(a) ^ b ^ c ^ F2(d);
      }
    }
  }
}

void expand(const uint8_t* k, uint8_t* rk) {
  memcpy(rk, k, 16);
  uint8_t rc = 1;
  for (int i = 16; i < 176; i += 4) {
    uint8_t t[4]; memcpy(t, rk + i - 4, 4);
    if (i % 16 == 0) {
      uint8_t x = t[0]; t[0] = sbox[t[1]] ^ rc; t[1] = sbox[t[2]]; t[2] = sbox[t[3]]; t[3] = sbox[x];
      rc = F2(rc);
    }
    for (int j = 0; j < 4; j++) rk[i+j] = rk[i-16+j] ^ t[j];
  }
}

py::bytes decrypt(py::bytes enc, py::bytes key, py::bytes iv, size_t offset) {
  std::string d = enc, k = key, v = iv;
  uint8_t rk[176], ctr[16], ks[16];
  expand((uint8_t*)k.data(), rk);
  memcpy(ctr, v.data(), 16);

  uint64_t num_blocks = offset / 16;
  for (int j = 15; j >= 8; j--) {
    uint64_t val = ctr[j] + (num_blocks & 0xFF);
    ctr[j] = (uint8_t)(val & 0xFF);
    num_blocks = (num_blocks >> 8) + (val >> 8);
  }

  for (size_t i = 0; i < d.size(); i += 16) {
    memcpy(ks, ctr, 16);
    aes_block(ks, rk);
    for (size_t j = 0; j < 16 && (i+j) < d.size(); j++) d[i+j] ^= ks[j];
    for (int j = 15; j >= 8; j--) if (++ctr[j]) break;
  }
  return py::bytes(d);
}

PYBIND11_MODULE(megadec, m) {
  m.def("decrypt", &decrypt);
}
