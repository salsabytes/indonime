...
def decode(enc, key, iv, offset):
  ...
  ctr = bytearray(v)
  num_blocks = offset // 16
  for j in range(15, 7, -1):
    val = ctr[j] + (num_blocks & 0xFF)
    ctr[j] = val & 0xFF
...
if __name__ == "__main__":
  ...
  iv = bytes(16)
  plain = b"Hello World!!!!!"
  ciph = decode(plain, key, iv, 0)
  assert decode(ciph, key, iv, 0) == plain, "AES-128 CTR self-check failed"
  print("(ok) videodec self-check passed")
