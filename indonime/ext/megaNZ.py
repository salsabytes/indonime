...
def resolve_mega_file(url, file_id, console):
  """Download+decrypt entire MEGA file → local path."""
  if not _decryptor:
...
def resolve_mega_file_parallel(url, file_id, console, num_connections=3, early_mb=2, chunk_mb=4):
  """Download+decrypt MEGA via parallel Range requests.

  Returns (temp_path, ready_event, stop_event, monitor_thread, progress_info).
  progress_info: mutable [downloaded, total] updated in realtime.
  """
  if not _decryptor:
    console.print("[red]✘ Tidak ada AES backend. Install: pip install cryptography[/red]")
    return None
